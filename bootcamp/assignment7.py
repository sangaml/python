...
print("Creating Public RouteTable...")
...
print("Creating Private RouteTable...")
...
web_sg_rule = ec2.authorize_security_group_ingress(
    ...
    IpPermissions=[
        ...
        },
        {
            ...
            'UserIdGroupPairs': [
                {
                    'Description': 'Allow Traffic only from db server',
                    'GroupId': db_sg_id
                },
            ]
        },
...
print("Creating db EC2 instance...")
ec2 = boto3.client('ec2')
db_server = ec2.run_instances(
    BlockDeviceMappings=[
        {
            'DeviceName': '/dev/sdh',
            'Ebs': {
...
instanceid_db = (db_server['Instances'])
for i in instanceid_db:
    db_instance_id = (i['InstanceId'])
ec2.create_tags(Resources=[db_instance_id],Tags=[{"Key": "Name", "Value": "db_Server"}])
...
time.sleep(90)
ec2.create_tags(Resources=[volume_id],Tags=[{"Key": "Name", "Value": "db server volume"}])
attach_volume = ec2.attach_volume(
    Device='/dev/sdf',
